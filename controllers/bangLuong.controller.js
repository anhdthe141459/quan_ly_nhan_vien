...
const getLuongNhanVienTheoThang = async (req, res) => {
  try {
    ...
    const bangLuongs = await bangLuongService.getLuongNhanVienTheoThang(
      year,
      month
    );
    res.json(bangLuongs);
...
const searchBangLuong = async (req, res) => {
  try {
    ...
    const bangLuongs = await bangLuongService.searchBangLuong(query);
    res.json(bangLuongs);
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
